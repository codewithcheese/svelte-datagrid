...
We chose fixed height for simplicity and performance. Variable height may be added in the future.
...
export function calculateVisibleRange(
  ...
  return {
    ...
    endIndex: Math.min(totalRows, start + visibleCount + overscan)
  };
}
```
...
We throttle scroll events to prevent excessive re-renders:
...
Rows need stable keys for Svelte's keyed each:

```svelte
{#each visibleRows as row, i (getRowId(row, startIndex + i))}
  <Row {row} />
{/each}
```

Without stable keys, Svelte would destroy and recreate DOM nodes during scroll, hurting performance.
...
- [Reference: DataGrid](../reference/datagrid.md) - rowHeight, overscan props
